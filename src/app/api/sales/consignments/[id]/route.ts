...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // meta is already included in consignment.lines — no extra query needed
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (updateLineMeta?.length) {
      for (const um of updateLineMeta) {
        const lineRow = await db.consignmentLine.findUnique({ where: { id: um.lineId }, select: { meta: true } });
        const newMeta = {
          ...parseMeta(lineRow?.meta ?? null),
        ...
        await db.consignmentLine.update({ where: { id: um.lineId }, data: { meta: JSON.stringify(newMeta) } });
      }
    }
    ...
    if (soldLines?.length) {
      for (const sl of soldLines) {
        ...
        if (meta.unit !== "WEIGHT") {
          ...
          if (sl.qtySold > maxCanSell) throw new Error(`Cannot mark ${sl.qtySold} as sold — only ${maxCanSell} pieces pending`);
          await db.consignmentLine.update({
            where: { id: sl.lineId },
            data: { qtySold: { increment: sl.qtySold }, ...(sl.actualPricePerUnit && { actualPricePerUnit: sl.actualPricePerUnit }) },
          });
        } else {
          if (sl.actualPricePerUnit) {
            await db.consignmentLine.update({ where: { id: sl.lineId }, data: { actualPricePerUnit: sl.actualPricePerUnit } });
          }
        }

        if (sl.weightSold !== undefined && sl.weightSold > 0) {
          const newMeta = { ...meta, weightSold: meta.weightSold + sl.weightSold };
          await db.consignmentLine.update({ where: { id: sl.lineId }, data: { meta: JSON.stringify(newMeta) } });
        }
    ...
    if (returnLines?.length) {
      for (const rl of returnLines) {
        ...
        if (meta.unit !== "WEIGHT") {
          ...
          if (rl.qtyReturned > maxCanReturn) throw new Error(`Cannot return ${rl.qtyReturned} — only ${maxCanReturn} pieces pending`);
          await db.consignmentLine.update({ where: { id: rl.lineId }, data: { qtyReturned: { increment: rl.qtyReturned } } });
          await db.inventoryItem.update({ where: { id: line.itemId }, data: { qtyPieces: { increment: rl.qtyReturned } } });
        }
        ...
        if (rl.weightReturned !== undefined && rl.weightReturned > 0) {
          ...
          await db.consignmentLine.update({ where: { id: rl.lineId }, data: { meta: JSON.stringify(newMeta) } });
        }
    ...
    // ── Fetch all lines for status + invoice computation ───────────────────
    const allLines = await db.consignmentLine.findMany({
      where: { consignmentId: id },
      select: { qtyIssued: true, qtySold: true, qtyReturned: true, actualPricePerUnit: true, estPricePerUnit: true, meta: true },
    });
    ...
    if (!newStatus && (soldLines?.length || returnLines?.length || updateLineMeta?.length)) {
      ...
      const weightPending = allLines.reduce((s, l) => {
        ...
        if (m.unit === "WEIGHT" || m.unit === "BOTH") return s + (m.weightIssued - m.weightSold - m.weightReturned);
        return s;
      }, 0);

      const existing = await db.consignment.findUnique({ where: { id }, select: { status: true } });
        ...
        if (conData) {
          ...
          if (!existingInv) {
            ...
            await db.invoice.create({
              data: { invoiceNo: `INV-${conData.consignmentNo}`, type: "CONSIGNMENT", consignmentId: id, amountTotal: roundedTotal, currency: "INR", paymentDays: 90, dueDate, status: "PENDING" },
            });
          } else {
            const newInvStatus = existingInv.amountPaid >= roundedTotal ? "PAID" : existingInv.amountPaid > 0 ? "PARTIAL" : "PENDING";
            await db.invoice.update({ where: { id: existingInv.id }, data: { amountTotal: roundedTotal, status: newInvStatus } });
          }
        }
      }
    }
  ...
  const result = await db.consignment.update({
    where: { id },
    data: { ...(data.status && { status: data.status }), ...(data.notes !== undefined && { notes: data.notes }) },
  });
  ...
  if (returnLines?.length) {
    try {
      ...
      if (wa.brokers) {
        const con = await db.consignment.findUnique({
          where: { id },
          select: { consignmentNo: true, broker: { select: { phoneWhatsapp: true, notifyWhatsapp: true } } },
        });
        if (con?.broker?.notifyWhatsapp && con.broker.phoneWhatsapp) {
          const totalReturned = returnLines.reduce((sum, rl) => sum + rl.qtyReturned, 0);
          const today = new Date().toLocaleDateString("en-IN");
          await queueWhatsapp(con.broker.phoneWhatsapp, `Return confirmed for ${con.consignmentNo}: ${totalReturned} pieces received back on ${today}.`, id, "CONSIGNMENT");
        }
      }
    } catch { /* WhatsApp failure must never fail the response */ }
  }
